...
public:
    ...
    Nodo(int grado, bool hoja) {
        claves = new int[2 * grado - 1];  // Máximo número de claves
        hijos = new Nodo*[2 * grado];     // Máximo número de hijos
        numClaves = 0;
        esHoja = hoja;
    }
...
void Nodo::tomarDeAnterior(int idx) {
    Nodo* hijo = hijos[idx];
...
void Nodo::tomarDeSiguiente(int idx) {
    Nodo* hijo = hijos[idx];
    Nodo* hermano = hijos[idx + 1];
